(()=>{"use strict";class e{constructor(e,t){window.addEventListener("popstate",(e=>this.route())),this._routes=e,this._defaultRoute=t,this._actualRoute=""}getActualRoute(){return this._actualRoute}route(){let e=!1;const t=window.location.hash;for(const[s,r]of Object.entries(this._routes))if("#"+s==t){this._actualRoute=s,r.init()&&(e=!0);break}e||this.routeDefault()}routeTo(e){let t=!1;for(const[s,r]of Object.entries(this._routes))s==e&&(t=!0);t?(this._actualRoute=e,window.location.hash="#"+e,this._routes[e].init()):console.log("route %s does not exist",e)}routeDefault(){this._actualRoute=this._defaultRoute.name,window.location.hash="#"+this._defaultRoute.name,this._defaultRoute.control.init()}}class t{constructor(e,t){this.name=e,this.storedClasses={},this.data=[],t.forEach((e=>{this.storedClasses[e.name]=e}))}getData(){return this.data}load(){let e=window.localStorage.getItem(this.name);null==e?this.save():this.data=JSON.parse(e,((e,t)=>{if("object"==typeof t&&"_class"in t){let e=new this.storedClasses[t._class];return Object.assign(e,t)}return t}))}save(){window.localStorage.setItem(this.name,JSON.stringify(this.data))}}class s{constructor(e){this._app=e}init(){return!0}}class r{constructor(){this._navEl=document.querySelector("header nav"),this._userEl=document.getElementById("user"),this._user=""}init(e){this._navEl.innerHTML=this._nav(e),this._userEl.innerHTML=this._user}_nav(e){return String.raw`
        ...
        `}}class p{constructor(){this._calendarWrapperEl=null,this._calendarBodyEl=null}init(e,t,s){this._calendarWrapperEl=document.getElementById("calendar"),this._calendarWrapperEl.innerHTML=this._calendar(e),this._calendarBodyEl=document.querySelector("tbody"),this._calendarBodyEl.innerHTML=this._calendarBody(e,t,s)}selectCell(e){let t=document.getElementById("day"+String(e));null!=t&&t.classList.add("selectedTab")}deselectCell(e){let t=document.getElementById("day"+String(e));null!=t&&t.classList.remove("selectedTab")}_calendar(e){return String.raw`
            <table>
                ...
                <tr>
                    ${_(c.getDayShorts().map((e=>String.raw`
                        <td>
                            <div>
                                ${e}
                            </div>
        ...
        `}_calendarBody(e,t,s){let r=this._calendarCells(e,t,s);return String.raw`
            ${_(r.map((e=>String.raw`
	            <tr>
                    ${_(e)}
                </tr>
            `)))}
        `}_calendarCells(e,t,s){let r=c.createDate(1,e.month,e.year),i=r.getDayInWeek(),a=c.createCurrentDate(),n=[],l=r.getDaysInMonth(),o=2-i;for(;l>0;){let r=[];for(let i=0;i<c.getDayShorts().length;i++){if(o<1||l<=0)r.push(this._otherMonthCell());else{let i=!1;t==o&&(i=!0),o==a.day&&e.month==a.month&&e.year==a.year?r.push(this._cell(i,"todayTab",o)):s.includes(o)?r.push(this._cell(i,"noteTab",o)):r.push(this._cell(i,"activeCellTab",o)),l--}o++}n.push(r)}return n}_cell(e,t,s){return String.raw`
        ...
        `}}class L extends s{constructor(e,t){super(e),this._view=new N,this._calendar=t}init(){return this._view.init(),this.update(),!0}update(){if(-1==this._calendar.getSelectedDay())this._view.clearNote();else{let e=this._findNote();null==e?this._view.noneNote():this._view.existingNote(e.name,e.description)}let e=this._calendar.getDate().makeCopy();this._calendar.noteDays=this._app.user.diaryNotes.filter((t=>t.date.month==e.month)).map((e=>e.date.day)),null==this._calendar.noteDays&&(this._calendar.noteDays=[]),this._calendar.update()}submitForm(e){if(-1!=this._calendar.getSelectedDay())if("createNote"==e.name)this._view.createNote();else if("create"==e.name||"safeChanges"==e.name){let e=this._calendar.getDate().makeCopy(),t=this._view.getNameInput(),s=this._view.getDescriptionInput();this._view.clearErrors();let r=x.isNameIncorrect(t);if(r)return void this._view.setNameError(r);if(r=x.isDescriptionIncorrect(s),r)return void this._view.setDescriptionError(r);let i=x.createDiaryNote(e.makeCopy(),t,s),a=this._app.user.diaryNotes,n=a.findIndex((t=>e.compare(t.date)));n<0?a.push(i):a[n]=i,this._app._users.save(),this.update()}else if("change"==e.name){let e=this._findNote();null!=e&&this._view.changeNote(e.name,e.description)}else if("delete"==e.name){let e=this._calendar.getDate().makeCopy(),t=this._app.user.diaryNotes;this._app.user.diaryNotes=t.filter((t=>!t.date.compare(e))),this._app._users.save(),this.update()}else this.update();else this.update()}_findNote(){return this._app.user.diaryNotes.find((e=>this._calendar.getDate().compare(e.date)))}}class M extends I{constructor(e){super(e),this._view=new F,this._calendar=new y(e),this._diaryNote=new L(e,this._calendar)}init(){let e=super.init();return e&&(this._view.init(),this._calendar.init(),this._diaryNote.init(),this._view.getForm().addEventListener("submit",(e=>{e.preventDefault(),this._submitForm(e.submitter)}))),e}_submitForm(e){this._calendar.submitForm(e),this._diaryNote.submitForm(e)}}class T extends I{constructor(e){super(e)}init(){return super.init()&&(this._app.user=null),!1}}class P{constructor(){this._headlineEl=document.querySelector("header h1"),this._mainEl=document.querySelector("main"),this._changeStyleFormEl=null,this._headline="Change style"}init(e){this._headlineEl.innerHTML=this._headline,document.title=this._headline,this._mainEl.innerHTML=this._changeStyleForm(e),this._changeStyleFormEl=document.querySelector("form")}getForm(){return this._changeStyleFormEl}_changeStyleForm(e){return String.raw`
            <form method="post" action="">
                <label>
        ...
        `}}class C extends I{constructor(e){super(e),this._view=new P}init(){let e=super.init();return e&&(this._view.init(this._app.user.style),this._view.getForm().addEventListener("submit",(e=>{e.preventDefault(),this._submitForm()}))),e}_submitForm(){let e=this._view.getForm(),t=this._app.user,s=new FormData(e);t.style=s.get("style"),this._app._users.save(),a(t.style)}}class k extends l{constructor(){super(),this.name="",this.description=""}static createNote(e,t){let s=new k;return s.name=e,s.description=t,s}static isNameIncorrect(e){return e.length>100?"The name is too long.":e.length<1?"The name must be at least one character.":""}static isDescriptionIncorrect(e){return e.length>2e4?"The description is too long.":""}}new class{constructor(){let s={main:new g(this),login:new v(this),register:new D(this),diary:new M(this),logout:new T(this),changeStyle:new C(this)},r={name:"main",control:s.main};this.router=new e(s,r),this.router.route(),this._users=new t("diary-js-users",[b,c,k,x]),this._users.load(),this.user=null}getUsers(){return this._users}}})();
